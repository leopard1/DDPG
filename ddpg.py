...
class Agent:
    ...
    def __init__(self, dimO, dimA):
        ...
        #
        self.theta_L = nets.theta(dimO[0], dimA[0] * dimA[0], 'theta_L', 3e-4)
        self.theta_U = nets.theta(dimO[0], dimA[0], 'theta_U', 3e-3)
        self.theta_V = nets.theta(dimO[0], 1, 'theta_V', 3e-4)
        # self.theta_Lt, update_Lt = exponential_moving_averages(self.theta_L, tau)
        # self.theta_Ut, update_Ut = exponential_moving_averages(self.theta_U, tau)
        self.theta_Vt, update_Vt = exponential_moving_averages(self.theta_V, tau)

        obs = tf.placeholder(tf.float32, [None] + dimO, "obs")
        act_test, sum_p = nets.ufunction(obs, self.theta_U)
        ...
        # test
        q, sum_q = nets.qfunction(obs, [0.], self.theta_V)
        ...
        # q
        lmat, sum_lmat = nets.lfunction(obs, self.theta_L)
        uvalue, sum_uvalue = nets.ufunction(obs, self.theta_U)
        avalue, sum_avalue = nets.afunction(act_train, lmat, uvalue, dimA[0])
        q_train, sum_qq = nets.qfunction(obs, avalue, self.theta_V)

        # q targets
        q2, sum_q2 = nets.qfunction(obs2, [0.] * FLAGS.bsize, self.theta_Vt)
        ...
        wd_q = tf.add_n([ql2 * tf.nn.l2_loss(var) for var in theta])  # weight decay
        loss_q = ms_td_error + wd_q
        # q optimization
        optim_q = tf.train.AdamOptimizer(learning_rate=lrq)
        grads_and_vars_q = optim_q.compute_gradients(loss_q, var_list=theta)
        ...
        log_act = [] if dimA[0] > 20 else [tf.histogram_summary(
            "act/inf" + str(i), act_test[:, i]) for i in range(dimA[0])]
        log_misc = [sum_qq, tf.histogram_summary("td_error", td_error)]
        log_grad = [grad_histograms(grads_and_vars_q)]
        log_train = log_obs + log_act + log_misc + log_grad
        ...
        with self.sess.as_default():
            ...
            self._train = Fun([obs, act_train, rew, obs2, term2], [train_q], log_train, self.writer)
